...
def extract_multiscale( net, img, detector, scale_f=2**0.25, 
    ...
    # Y = torch.cat(Y)
    # X = torch.cat(X)
    Y = torch.tensor([4 * i for i in range(H//4) for j in range(W//4)], dtype=torch.float32)
    X = torch.tensor([4 * j for i in range(H//4) for j in range(W//4)], dtype=torch.float32)
    S = torch.ones_like(X)
    descriptors = res['descriptors'][0]
    D = torch.stack([descriptors[0,:,int(Y[i]),int(X[i])] for i in range(len(X))])
